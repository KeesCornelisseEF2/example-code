...
class NodeTermsImporter extends AbstractImporter
{
...
//        $nCats = [];
//        foreach ($cats as $cat) {
//          $nCats[] = ['target_id' => $this->findOrCreateTaxonomyTerm('categorie', trim($cat))];
//        }
//        $node->set('field_categorie', $nCats);
//      }
//    }
//
//    if (!empty($data['tag'])) {
...
//        $nTags = [];
//        foreach ($tags as $tag) {
//          $nTags[] = ['target_id' => $this->findOrCreateTaxonomyTerm('tags', trim($tag))];
//        }
//        $node->set('field_tags', $nTags);
//      }
//    }
    ...
    if (!empty($data['plaatsnaam'])) {
      ...
      if (count($places)) {
        ...
        foreach ($places as $placeName) {
            $placeName = ucfirst(strtolower($placeName));
            $nPlaces[] = ['target_id' => $this->findOrCreateTaxonomyTerm('plaatsnamen', trim($placeName))];
        }
        $node->set('field_plaatsnaam_filter', $nPlaces);
      }
    }
